...
const SUPABASE_URL = (process.env.NEXT_PUBLIC_SUPABASE_URL || envContent.match(/NEXT_PUBLIC_SUPABASE_URL=(.+)/)?.[1] || '').trim();
const PROJECT_REF = process.env.SUPABASE_PROJECT_REF
    || envContent.match(/SUPABASE_PROJECT_REF=(.+)/)?.[1]?.trim()
    || SUPABASE_URL.match(/https?:\/\/([^.]+)\.supabase\.co/)?.[1];
const POOLER_REGION = (process.env.SUPABASE_POOLER_REGION || envContent.match(/SUPABASE_POOLER_REGION=(.+)/)?.[1] || 'us-east-1').trim();
...
if (!PROJECT_REF) {
    console.error('ERROR: Could not determine Supabase project ref. Set SUPABASE_PROJECT_REF or NEXT_PUBLIC_SUPABASE_URL.');
    process.exit(1);
}

// Supabase connection pooler (Supavisor) - uses JWT auth
// Transaction mode on port 6543
const connectionString = `postgresql://postgres.${PROJECT_REF}:${SERVICE_KEY}@aws-0-${POOLER_REGION}.pooler.supabase.com:6543/postgres`;
