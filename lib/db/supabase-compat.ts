...
class QueryBuilder implements PromiseLike<{ data: any; error: any; count: number | null }> {
  ...
  private async resolveEmbeds(
    rows: Row[],
    parsed: ParsedSelect,
    // Nested embeds must use the *parent embed table*, not the root query
    // table. Otherwise `orders → order_items → products` looks up products
    // as if they belonged to orders (WHERE order_id IN …) and blows up.
    ...
    for (const embed of parsed.embeds) {
      ...
      if (fk) {
        ...
        const edge = this.findReverseEdge(embedTable, table);
  ...
  private embedColumns(
    parsed: ParsedSelect,
    extraFk?: string,
    // When fetching rows that themselves have nested embeds, include the
    // forward FK columns those embeds need (e.g. order_items.product_id).
    table?: string
  ): string {
    if (parsed.star && parsed.columns.length === 0) return "*";
    const cols = new Set<string>();
