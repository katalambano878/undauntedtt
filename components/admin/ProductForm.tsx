...
import { getSkuPrefix, getSiteName } from '@/lib/site-defaults';
...
export default function ProductForm({ initialData, isEditMode = false }: ProductFormProps) {
    ...
    // Live SEO preview values (mirror what Google / OG will show)
    const storeOrigin = useMemo(getStoreOrigin, []);
    const slugPreview = slugify(urlSlug || productName);
    const previewUrl = slugPreview
        ? `${storeOrigin}/product/${slugPreview}`
        : `${storeOrigin}/product/...`;
    const previewTitle = (seoTitle || productName || 'Product title').trim();
    const previewDescription = (
        metaDescription
        || (description || '').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim()
        || `Shop ${productName || 'this piece'} at ${getSiteName()}.`
    ).slice(0, 200);
    ...
    const handleSubmit = async () => {
        try {
            ...
            const finalSlug = slugify(urlSlug || productName);
            if (!finalSlug) {
                alert('Please enter a product name or URL slug.');
                setLoading(false);
                return;
            }
            // Reflect the cleaned slug back to the field so admin sees what got saved
            ...
            if (/duplicate key/i.test(msg) && /slug/i.test(msg)) {
                alert(
                    `That URL slug is already in use by another product. ` +
                    `Pick a unique slug in the SEO tab (e.g. add a suffix like -2).`
                );
                setActiveTab('seo');
            } else {
                alert(`Error: ${msg}`);
                    ...
                    {activeTab === 'seo' && (
                        <div className="space-y-6 max-w-3xl">
                            <div>
                                ...
                                <p className="text-gray-600">Optimize how this product appears in Google, social shares, and rich results.</p>
                            </div>
                            ...
                            <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[1.05rem] leading-snug text-blue-700 font-medium hover:underline cursor-pointer truncate">
                                        {previewTitle}
                                    </p>
                            ...
                            <div>
                                ...
                                <p className={`text-sm mt-2 ${seoTitle.length > 60 ? 'text-amber-600' : 'text-gray-500'}`}>
                                    {seoTitle.length}/60 recommended (max 120). Leave blank to use product name.
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none"
                                    placeholder="A short, compelling summary that appears under the title in search results."
                                />
                                <p className={`text-sm mt-2 ${metaDescription.length > 160 ? 'text-amber-600' : 'text-gray-500'}`}>
                                    {metaDescription.length}/160 recommended. Leave blank to auto-use the product description.
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex items-stretch">
                                    <span className="text-gray-600 bg-gray-100 px-4 py-3 border-2 border-r-0 border-gray-300 rounded-l-lg text-sm whitespace-nowrap flex items-center max-w-[60%] truncate">
                                        {storeOrigin || 'your-store.com'}/product/
                                    </span>
                                    <input
                                        type="text"
                                        value={urlSlug}
                                        onChange={(e) => setUrlSlug(e.target.value)}
                                        onBlur={(e) => setUrlSlug(slugify(e.target.value))}
                                        className="flex-1 px-4 py-3 border-2 border-gray-300 rounded-r-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-mono text-sm"
                                        placeholder="product-slug"
                                    />
                                </div>
                                {urlSlug && urlSlug !== slugify(urlSlug) && (
                                    <p className="text-sm text-amber-600 mt-2 flex items-center">
                                        <i className="ri-alert-line mr-1"></i>
                                        Will be saved as <code className="bg-amber-50 px-1.5 py-0.5 rounded ml-1">{slugify(urlSlug)}</code>
                                    </p>
                                )}
                                <p className="text-sm text-gray-500 mt-2">
                                    Lowercase letters, numbers, and hyphens only. Slugs must be unique across all products.
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                    placeholder="gold necklace, jewelry Ghana, handmade"
                                />
                                <p className="text-sm text-gray-500 mt-2">Separate keywords with commas. Saved as product tags.</p>
                            </div>
                        </div>
                    )}
                </div>
